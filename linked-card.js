...
function getFirstInnerLabel(card, depth = 0) {
  if (depth > 2) return null;
  // Check nested cards (vertical-stack, horizontal-stack, grid, etc.)
  for (const child of card.cards ?? []) {
    const label = child.name || child.title || child.heading;
    if (label) return label;
    const nested = getFirstInnerLabel(child, depth + 1);
  ...
  // Check inner card (conditional, etc.)
  if (card.card) {
    const label = card.card.name || card.card.title || card.card.heading;
    if (label) return label;
  }
  return null;
}
...
function getCardLabel(card, index) {
  ...
  const ownLabel = card.name || card.title || card.heading;
  if (ownLabel) return ownLabel;
  if (card.entity) return `${card.type} (${card.entity})`;
  const innerLabel = getFirstInnerLabel(card);
  if (innerLabel) return innerLabel;
  return `${card.type || 'card'} #${index + 1} (unnamed)`;
}
...
function findCardByIdInList(cards, cardId) {
  for (let i = 0; i < (cards ?? []).length; i++) {
    const card = cards[i];
    if (getCardId(card, i) === cardId) return card;
  }
  return null;
}
...
function findCardByIdInView(view, cardId) {
  ...
  const inCards = findCardByIdInList(view?.cards, cardId);
  if (inCards) return inCards;
  // Also search inside sections (sections layout)
  for (const section of view?.sections ?? []) {
    const inSection = findCardByIdInList(section.cards, cardId);
            ...
            setTimeout(() => {
              try {
                ...
                try {
                  ...
                  if (cardEl) {
                    // The edit button lives on HUI-CARD-EDIT-MODE, the parent of hui-card
                    const editModeWrapper = cardEl.parentElement ?? cardEl.getRootNode()?.host;
                    const editBtn =
                      editModeWrapper?.shadowRoot?.querySelector('ha-icon-button') ??
                      editModeWrapper?.querySelector('ha-icon-button');
                    if (editBtn) {
                      editBtn.click();
                    } else {
                      // Fallback: fire ll-edit-card on the view
